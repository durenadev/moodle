...
class get_badge extends external_api {
    ...
    public static function execute(int $id): array {
        ...
        if (empty($CFG->enablebadges)) {
            $warnings[] = [
                'warningcode' => 'badgesdisabled',
                'message'     => get_string('badgesdisabled', 'badges'),
            ];
            return [
                'badge'    => $result,
                'warnings' => $warnings,
        ...
        // Manage the badge.
        if (isset($badgeclass->badge) && !empty($badgeclass->badge)) {
            $result[] = badges_prepare_badgeclass_for_external($badgeclass);
        } else {
            $warnings[] = [
                'item'        => $id,
                'warningcode' => 'relatedbadgedoesntexist',
                'message'     => get_string('error:relatedbadgedoesntexist', 'badges'),
            ];
        }
    ...
    public static function execute_returns(): external_single_structure {
        return new external_single_structure([
            'badge'  => new external_multiple_structure(
                badgeclass_exporter::get_read_structure()
            ),
