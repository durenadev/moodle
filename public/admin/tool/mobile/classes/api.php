...
class api {
     ...
     *
     * @param  bool $usecache Whether to use the cached value or to request it again to the Apps Portal. Defaults to true.
     ...
     */
    public static function get_subscription_information($usecache = true, $timeout = 10): ?array {
        global $CFG;

        // Use application cache to prevent multiple requests.
        ...
        // Get the current language and its parent to send to the WS.
        $settingslang = current_language();
        $parentlang = get_parent_language($settingslang);
        if (!empty($parentlang)) {
            $settingslang = $parentlang;
        }
        ...
        $args = [
            (object) [
                ...
                'args' => $fnparams,
                'moodlewssettinglang' => $settingslang,
            ]
        ];
